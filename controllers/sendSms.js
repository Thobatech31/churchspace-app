...
const { customSmsSchema, sendSmsSchema } = require("../middleware/authentication/sendsmsDojah_validation_schema")
...
const sendCustomSms = async (req, res) => {
    ...
    try {
        await customSmsSchema.validateAsync(req.body);
...
//Send to All Members
const sendSmsAllMembers = async (req, res) => {
    const { sender_id, mobile, message, select_all_members, select_all_firsttimers, departments, church_grp
    } = req.body;
    if (!sender_id) return res.status(400).json({ msg: "Sender ID Field is Empty" });
    if (!message) return res.status(400).json({ msg: "Message Field is Empty" });
    ...
    if (select_all_firsttimers) {
        ...
        if (all_first_timers) {
            ...
            return res.status(400).json({ msg: "FirstTimer Not Found" })
        }
    }
...
module.exports = {  sendCustomSms, sendSmsAllMembers, getWalletBalance }
