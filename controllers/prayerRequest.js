...
const { prayerrequestSchema } = require("../middleware/authentication/prayerrequest_validation_schema")
...
const createPrayerRequest = async (req, res) => {
    ...
    try {
        await prayerrequestSchema.validateAsync(req.body)
...
const updatePrayerRequest = async (req, res) => {
    ...
    if (!availId) return res.status(400).json({ msg: "PrayerRequest with Id does not Exists" });
...
const deletePrayerRequest = async (req, res) => {
    const { id } = req.params;
    const availId = await PrayerRequest.findOne({ _id: id })
    if (!availId) return res.status(400).json({ msg: "Prayer Request with Id does not Exists" });
