...
const { churchGroupSchema } = require('../middleware/authentication/churchgroup_validation_schema');
//Create  CHURCH GROUP
const createChurchGroup = async (req, res) => {
    const { group, description } = req.body;

    const user = req.user;
    try {
        await churchGroupSchema.validateAsync(req.body);
...
const updateChurchGroup = async (req, res) => {
    const { id } = req.params;
    const availId = await Group.findOne({ _id: id })
    if (!availId) return res.status(400).json({ msg: "Group with Id does not Exists" });
...
const deleteChurchGroup = async (req, res) => {
    const { id } = req.params;
    const availId = await Group.findOne({ _id: id })
    if (!availId) return res.status(400).json({ msg: "Group with Id does not Exists" });
...
//Get Group
const getSingleChurchGroup = async (req, res) => {
    const { id } = req.params;
    const availId = await Group.findOne({ _id: id })
    if (!availId) return res.status(400).json({ msg: "Group with Id does not Exists" });
    try {
...
const findChurchGroupById = async (req, res, next) => {
...
const getAllChurchGroup = async (req, res) => {
    //Initiating a seach parameter with (group)
    let query = {};
    if (req.query.search) {
        query.$or = [
...
module.exports = {
    createChurchGroup, updateChurchGroup, deleteChurchGroup,
    getSingleChurchGroup, findChurchGroupById, getAllChurchGroup}
