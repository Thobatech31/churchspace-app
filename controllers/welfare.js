...
const { welfareSchema }  = require("../middleware/authentication/welfare_validaton_schema");
...
const createWelfare = async (req, res) => {
    ...
    const user = req.user;
    try {
        await welfareSchema.validateAsync(req.body)
...
const uploadWelfareDoc = async (req, res) => {
    ...
    uploadSingle(req, res, async(err) =>{
        ...
        if (!availId) return res.status(400).json({ msg: "Welfare with Id does not Exists" });
...
//Get Welfare
const getSingleWelfare = async (req, res) => {
    const { id } = req.params;
...
// Get All Welfare
const getAllWelfare = async (req, res, next) => {
...
const findWelfareById = async (req, res, next) => {
    //Initiating a seach parameter with (Welfare)
    let query = {};
    if (req.query.search) {
        query.$or = [
...
module.exports = {  createWelfare, updateWelfare,
